...
# Aim is to attain high cohesion, low coupling
...
class Sale:
    ...
    # low coupling is achieved as payment_method can be anything
    # and can be extended to accept different payment options
...
def main() -> None:
    ...
    pp(f"Price of line 1: ${row1.total_line_price() / 100:.2f}")

    row2 = SaleLineItem(keyboard, quantity=3)
    pp(f"Price of line 2: ${row2.total_line_price() / 100:.2f}")
    ...
    cash = Cash()
    pp(f"Final value paid in cc: ${sale.total_discounted_price(cc) / 100:.2f}")
    pp(f"Final value paid in cash: ${sale.total_discounted_price(cash) / 100:.2f}")
